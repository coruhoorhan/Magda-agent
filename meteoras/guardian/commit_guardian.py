...
class CommitGuardian:
    ...
    def check_10_agent_signature(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            # Auto-provision if missing in autonomous headless run
            if not name:
                try:
                    subprocess.run(["git", "config", "user.name", "Meteoras Autonomous Agent"], cwd=self.repo_root, capture_output=True)
                    name = "Meteoras Autonomous Agent"
                except Exception:
                    pass

            if not email:
                try:
                    subprocess.run(["git", "config", "user.email", "agent@meteoras.ai"], cwd=self.repo_root, capture_output=True)
                    email = "agent@meteoras.ai"
                except Exception:
                    pass
...
# Meteoras Autonomous 10-Point Pre-Commit Guardian Hook
if command -v meteoras >/dev/null 2>&1; then
    meteoras check || exit 1
elif python3 -m meteoras.guardian.commit_guardian check 2>/dev/null; then
    :
else
    python3 -m magda_agent.guardian.commit_guardian check || exit 1
fi
"""

    commit_msg_content = """#!/usr/bin/env bash
# Meteoras Conventional Commits Linter Hook
if command -v meteoras >/dev/null 2>&1; then
    meteoras check-msg "$1" || exit 1
elif python3 -m meteoras.guardian.commit_guardian check-msg "$1" 2>/dev/null; then
    :
else
    python3 -m magda_agent.guardian.commit_guardian check-msg "$1" || exit 1
fi
"""
