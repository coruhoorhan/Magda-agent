...
import pytest
from magda_agent.gateway.canvas_live_v3 import CanvasLiveVisualizerV3

@pytest.fixture
def visualizer():
    return CanvasLiveVisualizerV3(host="127.0.0.1", port=9999)

@pytest.mark.asyncio
async def test_start_and_stop(visualizer):
    with patch("magda_agent.gateway.canvas_live_v3.serve", new_callable=AsyncMock) as mock_serve:
