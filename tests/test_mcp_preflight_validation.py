...
from magda_agent.safety.mcp_preflight_validation import MCPActionToolPreflightValidator
...
def validator(weather_tool_schema, mock_policy, mock_executor):
    val = MCPActionToolPreflightValidator(policy_layer=mock_policy, executor=mock_executor)
    val.register_tool_schema("get_weather", weather_tool_schema)
    return val


def test_register_invalid_schema():
    val = MCPActionToolPreflightValidator()
