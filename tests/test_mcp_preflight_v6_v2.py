"""Tests for MCP Action Tools Pre-flight Validation V6 v2."""

import json
import pytest
from unittest.mock import AsyncMock, MagicMock
from magda_agent.integration.mcp_preflight_v6_v2 import MCPPreflightValidatorV6V2, PreflightMCPServerWrapperV6V2
...
def validator(mock_registry):
    return MCPPreflightValidatorV6V2(registry=mock_registry, forbidden_tools=["nuke_system", "rm_rf"])
...
async def test_wrapper_delegates_valid_request():
    ...
    validator = MCPPreflightValidatorV6V2(registry=mock_registry)
    wrapper = PreflightMCPServerWrapperV6V2(server=mock_server, validator=validator)
